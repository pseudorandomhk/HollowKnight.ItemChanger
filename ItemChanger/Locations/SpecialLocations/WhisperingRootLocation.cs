    ...
    public class WhisperingRootLocation : AutoLocation, ILocalHintLocation
    {
        ...
        private static void HookRootFsm(PlayMakerFSM fsm)
        {
            ...
            fsm.GetState("Init").AddLastAction(new FsmLambda(() =>
            {
                ...
                GameObject dreamDialogue = fsm.FsmVariables.GetFsmGameObject("Dream Dialogue").Value;
                if (loc.GetItemHintActive() && dreamDialogue != null)
                {
                    HintBox.Create(dreamDialogue.transform, loc.Placement);
                }
            }));

            fsm.GetState("Grow Pause").AddFirstAction(new FsmLambda(() =>
            {
        ...
        private static void HookOrbFsm(PlayMakerFSM fsm) 
        {
            ...
            fsm.GetState("Collect").RemoveActionsOfType<IncrementPlayerDataInt>();
        }
